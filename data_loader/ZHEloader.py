...
class MRIANDCT(BaseDataLoader):
    ...
    def __init__(self, config, train = True, transform=None, target_transform=None,classify = True):
        super(MRIANDCT, self).__init__(config)
        self.path = config.data_path
        self.classify = classify
        self.model_type = config.exp_name 
        ...
        if self.train:
            if config.isSample:
                self.Num_train = config.Num_train
                (self.train_CTdata,self.train_MRdata, self.train_labels) = self.loadSampledData(self.train)
                print("load trainSample successful")
            else:
                (self.train_CTdata,self.train_MRdata, self.train_labels) = self.loadData(self.train)
                print("load train successful")
            ...
            if config.isSample:
                self.Num_test = config.Num_test
                (self.test_CTdata, self.test_MRdata, self.test_labels) = self.loadSampledData(self.train)
                print("load testSample successful")
            else:
                (self.test_CTdata, self.test_MRdata, self.test_labels) = self.loadData(self.train)
                print("load test successful")
    ...
    def loadSampledData(self, train):
        path = str(self.path) 
        if train:
            fp = open(os.path.join(path, "MRICT_train_dataSample.txt"), 'r')
        else:
            fp = open(os.path.join(path, "MRICT_test_dataSample.txt"), 'r')                
    ...
    def loadData(self, train):
        path = str(self.path) 
        if train:
            fp = open(os.path.join(path, "train_data.txt"), 'r')
        else:
            fp = open(os.path.join(path, "test_data.txt"), 'r')
        data = []  
        labels = []  
