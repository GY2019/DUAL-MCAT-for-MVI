...
def fittwo(train_loader, val_loader, model, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[],
        start_epoch=0,flag=0):
    """
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        cnf_mat=[]
        # Train stage
        train_loss, metrics = train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics,epoch,flag=flag)
...
def fit1(train_loader, val_loader, model, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[],
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        cnf_mat=[]
        # Train stage
        train_loss, metrics = train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics,epoch, obj_label=obj_label,flag=flag)

        message = 'Epoch: {}/{}. Train set: Average loss: {:.4f}. lr:{}'.format(epoch + 1, n_epochs, train_loss,lr)
        for metric in metrics:
            message += '\t{}: {}'.format(metric.name(), metric.value())
            temp1+=metric.value()
        val_loss, metrics,Sens, Prec, F1, cnf_mat = test_epoch(val_loader, model, loss_fn, cuda, metrics,epoch, obj_label=obj_label,flag=flag)
        val_loss /= len(val_loader)
...
def train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics,epoch,flag):
    acc = []
    sens = []
    spec = []
    auc = []
