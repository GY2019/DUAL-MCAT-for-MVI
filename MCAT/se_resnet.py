...
from MCAT.se_module import SELayer
...
class CifarSEBasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, modality=8):
        ...
        self.bn2 = nn.BatchNorm2d(planes)
        self.se = SELayer(planes, modality)
        if inplanes != planes:
...
class CifarSEResNet(nn.Module):
    def __init__(self, block, n_size, num_classes=10, modality=8):
        super(CifarSEResNet, self).__init__()
        self.inplane = 16
        self.conv1 = nn.Conv2d(modality, self.inplane, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.inplane)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16, blocks=n_size, stride=1, modality=modality)
        self.layer2 = self._make_layer(block, 32, blocks=n_size, stride=2, modality=modality)
        self.layer3 = self._make_layer(block, 64, blocks=n_size, stride=2, modality=modality)
    ...
    def _make_layer(self, block, planes, blocks, stride, modality):
        strides = [stride] + [1] * (blocks - 1)
        layers = []
        for stride in strides:
            layers.append(block(self.inplane, planes, stride, modality))
            self.inplane = planes
