...
# res_model = resnet.resnet20(3,num_features = 2, num_classes = config.classes)
# print(res_model)
...
# #optimizer = torch.optim.Adam(res_model.parameters(), lr=lr, weight_decay=1e-4)
# fit(train_loader, test_loader, model1, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[AccumulatedAccuracyMetric()])
...
# res_model = resnet.resnet20(3,num_features = 8, num_classes = config.classes)
# print(res_model)
...
# # Set up the network and training parameters
# from model.cifar_networks import TripletNet,classfication1Net
# from model.losses import TripletLoss
# res_model = resnet.resnet20(3,num_features = 64, num_classes = config.classes)
# print(res_model)
# model = TripletNet(res_model)

# model.cuda()
# loss_fn = TripletLoss(0.5).cuda()

# lr = 1e-3
# optimizer = torch.optim.Adam(res_model.parameters(), lr=lr, weight_decay=1e-4)
# scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 32, gamma=0.1, last_epoch=-1)
...
# # torch.save(res_model, './model/model_dict_ss.pth')
# # print("done")
# # model1 = torch.load('./model/model_dict_ss.pth')
# # for p in model1.parameters():
# #     p.requires_grad = False



# # for p in res_model.parameters():#添加分类器并进行微调 
# #      p.requires_grad = False
# train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, **kwargs)
# test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, **kwargs)
# loss_fn = nn.NLLLoss().cuda() 
# lr = 1e-4
# model1=resnet.ClassificationNet1(res_model.embedding_net, 16, config.classes)
# model1.cuda()
# optimizer = torch.optim.SGD(model.parameters(), lr, momentum=0.9, weight_decay=5e-4)  
# #optimizer = torch.optim.Adam(res_model.parameters(), lr=lr, weight_decay=1e-4)
# fit(train_loader, test_loader, model1, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[AccumulatedAccuracyMetric()])
...
# #载入预训练Resnet
# pretrain_dict = torch.load('model/model_dict_ss.pkl')
# res_model = resnet.resnet20(4,num_features = 2, num_classes = 10)
# res_model.load_state_dict(pretrain_dict)
...
# # 训练模型
# res_model = se_resnet20(num_classes=10, reduction=16)
# print(res_model)
...
for i in range(times):
    ...
    res_model = resnet.resnet20(3,num_features = 16, num_classes = config.classes)
    #res_model = se_resnet20(num_classes=32, reduction=16)
    print(res_model)
    model = TripletNet(res_model)

    model.cuda()
    loss_fn = TripletLoss(0.6).cuda()
    ...
    n_epochs = 200
    log_interval = 100                           
    c,d=fit(train_loader, test_loader, model, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval)
    ...
    model1=resnet.ClassificationNet1(res_model,16, config.classes)
    #model1=res_model
    model1.cuda()
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model1.parameters()), lr, momentum=0.9, weight_decay=5e-4) 
    #optimizer = torch.optim.SGD(model1.parameters(), lr, momentum=0.9, weight_decay=5e-4)  
    #optimizer = torch.optim.Adam(res_model.parameters(), lr=lr, weight_decay=1e-4)
    a,b=fit(train_loader, test_loader, model1, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[AccumulatedAccuracyMetric()])
...
#     # Set up the network and training parameters
#     res_model = resnet.resnet20(3,num_features = 16, num_classes = config.classes)
#     #res_model = se_resnet20(num_classes=3, reduction=16)
#     #print(res_model)
#     print("time:"+ str(i))
...
#     n_epochs = 200
#     log_interval = 100                           
#     a,b=fit(train_loader, test_loader, res_model, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[AccumulatedAccuracyMetric()])
#     avgtrain.append(a)
#     avgval.append(b)
# avgtrain1=0
# avgval1=0
