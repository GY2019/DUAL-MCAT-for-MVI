...
def train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics, obj_label,flag):
    ...
    if obj_label:
        for batch_idx, (data, target, obj_l) in enumerate(train_loader):
            ...
            # if flag==True:
            #     #acc, sens, spec, auc = accuracy(temp, outputs, 3, 0)
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            #     acc, sens, spec, auc = accuracy(temp, outputs[0].data.max(1, keepdim=True)[1], 3, 0)
            #     print acc, sens, spec, auc
            #     print temp[0], outputs[0].data.max(1, keepdim=True)[1]
            if batch_idx % log_interval == 0:
                message = 'Train: [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
    ...
    if obj_label:
        ...
        for batch_idx, (data, target) in enumerate(val_loader):
            ...
            if flag==True:
                ...
                l1, l2, l3, l4 = accuracy(temp, outputs[0].data.max(1, keepdim=True)[1], 3, 0)
                Sens.append(l1)
                Prec.append(l2)
                F1.append(l3)
                cnf_mat.append(l4)
