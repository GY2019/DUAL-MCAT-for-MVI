...
def extract_embeddings(dataloader, model):
    ...
    for images, target in dataloader:
        imagelbp = exactLBPfeatruebatch(images)
        images = Variable(images, volatile=True)
        imagelbp = Variable(imagelbp, volatile=True)
        if cuda:
            images = images.cuda()
            imagelbp = imagelbp.cuda()
        embeddings[k:k+len(images)] = model.get_embedding(images,imagelbp).data.cpu().numpy()[:, 0:2]
