...
def drawroc(eachmodel,data_loader,criterion,tprs,aucs,mean_fpr,tpr_list,fpr_list,bestAUC,flag=1):
    ...
    if roc_auc > bestAUC[0] :
      ...
      torch.save(model.state_dict(),'./CTbranchMCAT.pth')
...
def drawavgroc(tpr_list,fpr_list,aucs,mean_fpr,tprs):
    ...
    #plt.show()
    plt.savefig("./rocresult/CTROC.png")
...
wholedata_path=[]
fp = open(os.path.join(config.data_path, "CTdata.txt"), 'r')
line = fp.readline()
while len(line):
...
for train_index, test_index in kf.split(wholedata_path):
    ...
    # Load data
    print('Create the data generator.')
    train_dataset = MRIORCT(config, resamplelabel(DataAug(train_select)),train = True,classify = False,MRI = False)#MRI = False -- CT   
    test_dataset = MRIORCT(config, test_select, train = False,classify = False,MRI = False)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, **kwargs)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, **kwargs)

    res_model = se_resnet20(num_classes=64, modality=3)
    ...
    train_dataset = MRIORCT(config, train_select,train = True,classify = True,MRI = False)#MRI = False -- CT   
    test_dataset = MRIORCT(config, test_select, train = False,classify = True,MRI = False)
    ...
    if e > bestvalacc :
      ...
      #torch.save(model1.state_dict(),'./CTbranchMCAT.pth')
    elif e==bestvalacc:
      if a > besttrainacc :
          besttrainacc = a
          #print("saveing the {} zhe times model……".format(ZHE_times))
