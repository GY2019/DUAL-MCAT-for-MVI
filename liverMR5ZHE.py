...
from model.trainer import fit,fit1,fit2
...
def drawroc(eachmodel,data_loader,criterion,tprs,aucs,mean_fpr,tpr_list,fpr_list,bestAUC,flag=1):
    ...
    for i, (data, target) in enumerate(data_loader):#train_loader
    ...
    """
    y_label = np.array([0, 1, 1, 0, 1, 0, 1, 1])
    y_score = np.array([0.22465098, 0.79505259, 0.69359905, 0.36772281, 0.62433159, 0.31976736, 0.56209314, 0.380137])
    """
    print(y_label)
    print(y_score)
    ...
    if roc_auc > bestAUC[0] :
      ...
      torch.save(model.state_dict(),'./MRIbranchMCAT.pth')
...
def drawavgroc(tpr_list,fpr_list,aucs,mean_fpr,tprs):
    ...
    #plt.show()
    plt.savefig("./rocresult/MRROC.png")
...
wholedata_path=[]
fp = open(os.path.join(config.data_path, "MRIdata.txt"), 'r')
line = fp.readline()
while len(line):
...
for train_index, test_index in kf.split(wholedata_path):
    ...
    # Load data
    print('Create the data generator.')
    train_dataset = MRIORCT(config, resamplelabel(DataAug(train_select)),train = True,classify = False,MRI = True)#MRI = False -- CT   
    test_dataset = MRIORCT(config, test_select, train = False,classify = False,MRI = True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, **kwargs)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, **kwargs)

    res_model = se_resnet20(num_classes=64, modality=6)
    ...
    c, d, f, Sens1, Prec1, F11, cnf_mat1= fit2(train_loader, test_loader, res_model, loss_fn, optimizer, scheduler,
                                                  n_epochs, cuda, log_interval, flag=flag)
    
    flag = True
    train_dataset = MRIORCT(config, train_select,train = True,classify = True,MRI = True)#MRI = False -- CT   
    test_dataset = MRIORCT(config, test_select, train = False,classify = True,MRI = True)
    ...
    scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer1, step_size=5, gamma=0.85, last_epoch=-1)
    
    a, b, e, Sens1, Prec1, F11, cnf_mat1, k = fit(train_loader, test_loader, model1, loss_fn, optimizer1, scheduler1,
                                              n_epochs, cuda, log_interval, metrics=[AccumulatedAccuracyMetric()],
                                              flag=flag)
    Sens.append(max(Sens1))
    Prec.append(max(Prec1))
    F1.append(max(F11))
    print("Sens:",max(Sens1))
    print("Prec:",max(Prec1))
    print("F1:",max(F11))
    ...
    if e > bestvalacc :
      bestvalacc = e
      besttrainacc = a
      #print("saveing the {} zhe times model……".format(ZHE_times))
      #torch.save(model1.state_dict(),'./MRIbranchMCAT.pth')
    elif e==bestvalacc:
      if a > besttrainacc :
          besttrainacc = a
          #print("saveing the {} zhe times model……".format(ZHE_times))
    ...
    kwargs = {'num_workers': 1, 'pin_memory': True} if cuda else {}
    ZHE_times +=1
    drawroc(model1.cuda(),test_loader,nn.CrossEntropyLoss().cuda(),tprs,aucs,mean_fpr,tpr_list,fpr_list,bestAUC,flag=1)
